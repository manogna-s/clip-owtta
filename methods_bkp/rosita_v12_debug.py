...
def tta_id_ood(args, model, ID_OOD_loader, ID_classifiers):
    ...
    for i, (image, gt) in enumerate(ID_OOD_loader):
        ...
        if ood_thresh == 'otsu':
            ...
            coarse_thresh = threshold_range[np.argmin(criterias)]
            _, stats = compute_os_variance_stats(np.array(scores_q), coarse_thresh)

            threshold_range = np.arange(max(coarse_thresh-0.1,0), min(coarse_thresh+0.1,1), 0.001)
            criterias = []
            for th in threshold_range:
                inter_var, _ = compute_os_variance_stats(np.array(scores_q), th)
                criterias.append(inter_var)
            # criterias = [compute_os_variance(np.array(scores_q), th) for th in threshold_range]
            # print(scores_q, criterias, np.argmin(criterias))
            best_thresh = threshold_range[np.argmin(criterias)]
            _, stats = compute_os_variance_stats(np.array(scores_q), best_thresh)
            # print(f'coarse thresh: {coarse_thresh}; fine thresh: {best_thresh}; ({max(coarse_thresh-0.1,0)}, {min(coarse_thresh+0.1,1)})')
